...
const CLOCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(courseOfferingId ? `/clos?courseOffering=${courseOfferingId}` : '/clos');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to create CLO');
      console.error('Failed to create CLO:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Create Course Learning Outcome</h1>
            <p className="mt-1 text-sm text-gray-500">
              Define a new learning outcome for the course
            </p>
          </div>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-lg shadow">
        ...
        <div className="bg-gray-50 px-6 py-4 flex justify-end space-x-3 rounded-b-lg">
          <Button
            type="button"
            variant="outline"
            onClick={() => navigate(courseOfferingId ? `/clos?courseOffering=${courseOfferingId}` : '/clos')}
            icon={X}
          >
            Cancel
          </Button>
