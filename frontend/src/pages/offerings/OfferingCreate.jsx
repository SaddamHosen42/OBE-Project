...
const OfferingCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const response = await offeringService.createOffering(submitData);
      
      if (response.success) {
        // Navigate to offerings list
        navigate('/offerings', { 
          state: { message: 'Course offering created successfully' } 
        });
      }
    } catch (error) {
      console.error('Error creating offering:', error);
      setApiError(error.message || 'Failed to create course offering');
    } finally {
      setIsLoading(false);
    }
  };
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 mb-2">Create Course Offering</h1>
          <p className="text-gray-600">Add a new course offering for the semester</p>
        </div>
