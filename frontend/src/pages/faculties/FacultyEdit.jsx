...
const FacultyEdit = () => {
  ...
  useEffect(() => {
    const fetchFaculty = async () => {
      try {
        ...
        setApiError(error.message || 'Failed to load faculty data');
      } finally {
        setIsFetching(false);
      }
    };
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await facultyService.updateFaculty(id, formData);
      
      if (response.success) {
        // Navigate to faculty list
        navigate('/faculties', { 
          state: { message: 'Faculty updated successfully' } 
        });
      }
    } catch (error) {
      console.error('Error updating faculty:', error);
      setApiError(error.message || 'Failed to update faculty');
    } finally {
      setIsLoading(false);
    }
  };
      ...
      <div className="flex items-center justify-between mb-8">
        <div>
          <h1 className="text-3xl font-bold text-gray-900 flex items-center">
            <FiSave className="mr-3 text-purple-600" size={36} />
            Edit Faculty
          </h1>
          <p className="text-gray-600 mt-2">Update faculty information</p>
        </div>
