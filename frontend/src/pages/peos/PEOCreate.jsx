...
const PEOCreate = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      navigate(`/peos${degreeIdParam ? `?degree=${degreeIdParam}` : ''}`);
    } catch (error) {
      console.error('Error creating PEO:', error);
      setErrors({
        submit: error.response?.data?.message || 'Failed to create PEO'
      });
    } finally {
      setLoading(false);
    }
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-4">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Create Program Educational Objective</h1>
          <p className="mt-1 text-sm text-gray-500">
            Define a new PEO for your degree program
          </p>
        </div>
      </div>
