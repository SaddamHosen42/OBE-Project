...
import { Save, X, Target, Trash2 } from 'lucide-react';
...
const PLOEdit = () => {
  ...
  const { plo, loading: ploLoading, error: ploError, updatePLO, deletePLO } = usePLO(id);
  ...
  useEffect(() => {
    if (plo) {
      setFormData({
        degree_id: plo.degree_id || '',
        plo_code: plo.plo_code || '',
        description: plo.description || '',
        bloom_level_id: plo.bloom_level_id || '',
        peo_mappings: plo.peo_mappings?.map(mapping => mapping.peo_id) || []
      });
    }
  }, [plo]);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await updatePLO(formData);
      navigate(`/plos/${id}`);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to update PLO');
  ...
  const handleDelete = async () => {
    if (window.confirm('Are you sure you want to delete this PLO? This action cannot be undone.')) {
      try {
        ...
        setError(err.response?.data?.message || 'Failed to delete PLO');
    ...
    <div className="max-w-4xl mx-auto">
      <div className="mb-6 flex justify-between items-start">
        <div className="flex items-center space-x-3">
          ...
          <div>
            <h1 className="text-3xl font-bold text-gray-900">Edit Program Learning Outcome</h1>
            <p className="mt-1 text-sm text-gray-500">
              Update PLO information
            </p>
          </div>
        </div>
        <Button
