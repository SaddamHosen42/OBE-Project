...
import { Save, X, Award, Loader } from 'lucide-react';
...
const GradeScaleEdit = () => {
  const navigate = useNavigate();
  const { id } = useParams();
  const { updateGradeScale, getGradeScale } = useGradeScales();
  ...
  const fetchGradeScale = async () => {
    try {
      setLoading(true);
      const scale = await getGradeScale(id);
      ...
      // Format grade points for the form
      const formattedGradePoints = (scale.grade_points || []).map(gp => ({
        grade_point_id: gp.grade_point_id,
        grade: gp.grade,
        min_marks: gp.min_marks.toString(),
        max_marks: gp.max_marks.toString(),
        grade_point: gp.grade_point.toString(),
        description: gp.description || ''
      }));
      
      setGradePoints(formattedGradePoints);
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to fetch grade scale');
      console.error('Error fetching grade scale:', err);
    } finally {
      setLoading(false);
    }
  };
    ...
    try {
      const dataToSubmit = {
        ...
        grade_points: gradePoints.map(gp => ({
          grade_point_id: gp.grade_point_id, // Include ID for updates
          grade: gp.grade,
      ...
      await updateGradeScale(id, dataToSubmit);
      navigate('/grades');
    } catch (err) {
      setError(err.response?.data?.message || 'Failed to update grade scale');
      console.error('Error updating grade scale:', err);
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      <div className="flex items-center space-x-3">
        ...
        <div>
          <h1 className="text-3xl font-bold text-gray-900">Edit Grade Scale</h1>
          <p className="mt-1 text-sm text-gray-500">
            Update grading scale and grade points
          </p>
        </div>
      </div>
